...
import { db } from "@/db";
import { clients, invoices, companies, payments } from "@/db/schema";
import {
  processClientPaymentSettlementEvent,
  processPaymentEvents,
} from "@/lib/notifications/event-services";
import { getClientPaymentReceivedData } from "@/lib/notifications/notification-data";
import { notifyClientPaymentReceived } from "@/lib/notifications/notification-services";
...
export async function GET(req) {
  try {
    ...
    if (mode === "send") {
      // Find an existing payment if any
      const existingPayment = await db
        .select({ id: payments.id })
        .from(payments)
        .limit(1);
      const testPaymentId = existingPayment[0]?.id || null;

      let clientData = null;
      try {
        clientData = await getClientPaymentReceivedData({
          clientId,
          companyId: client?.companyId || null,
        ...
        return NextResponse.json({
          failedAt: "getClientPaymentReceivedData",
          error: err1.message,
          stack: err1.stack,
        });
      }
      ...
      try {
        const result = await notifyClientPaymentReceived(clientData || payload);
        return NextResponse.json({
          success: true,
        ...
        return NextResponse.json({
          failedAt: "notifyClientPaymentReceived",
          error: err2.message,
          stack: err2.stack,
        });
      }
