...
export default function SendInvoiceReminderModal({
    ...
    getSingleInvoiceReminderPreviewHtml({
      ...
      .then((res) => {
        if (res?.success) {
          ...
          const errMsg = res?.error || "Failed to render email preview";
          console.error("[InvoiceReminderModal] Preview error:", errMsg);
          setPreviewError(errMsg);
        }
      })
      .catch((err) => {
        console.error("[InvoiceReminderModal] Preview exception:", err);
        setPreviewError(
  ...
  }, [invoiceId, reminderType, customNote]);

  useEffect(() => {
    if (activeTab === "preview" && invoiceId) {
      loadPreview();
    }
  }, [activeTab, invoiceId, loadPreview]);
