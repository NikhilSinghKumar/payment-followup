...
export default function SendClientReminderModal({
    ...
    getClientStatementReminderPreviewHtml({
      ...
      .then((res) => {
        if (res?.success) {
          ...
          const errMsg = res?.error || "Failed to render statement preview";
          console.error("[ClientReminderModal] Preview error:", errMsg);
          setPreviewError(errMsg);
        }
      })
      .catch((err) => {
        console.error("[ClientReminderModal] Preview exception:", err);
        setPreviewError(
  ...
  }, [clientId, reminderType, customNote]);

  useEffect(() => {
    if (activeTab === "preview" && clientId) {
      loadPreview();
    }
  }, [activeTab, clientId, loadPreview]);
